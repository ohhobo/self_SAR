...
## SENet 
- 对特征进行动态加权
- 对离散特征做field-wise加权：若某特征的embedding是n维向量，称向量中的n个元素归为一个field,获得相同的权重；如果m个field那么权重向量是m维；不同特征的embedding维度可以不同，这个维度的不同可以理解为同的field的重要性不同
- row-wise multiply将矩阵的每一行和向量的每个元素相乘，得到一个与原矩阵shape相同的矩阵
- m个特征需要进行r的降维，再升维，类似unet
### Bilinear Cross
- 之前特征交叉有两种方式，内积和哈达玛乘积，哈达玛乘积需要人工选择少量向量参与，同时需要两个field形状相同
- Bilinear Cross可以不同field形状不同，需要几个矩阵进行转换
## FiBiNet
- SENet和Bilinear Cross的结合
- 直接拼接、BiLinear、SENet分别处理，然后再做拼接

# RAG
- 检索增强技术
- why：时效性不够、安全性限制、精确性需求
# 行为序列
- 用户最近对物品的点击、收藏、购买等行为序列，一般称为LastN，表示用户最近交互过n个物品
- 重点是用户行为序列建模的方法，重点关注用户特征的LastN行为序列
## 用户行为序列建模
### LastN特征
- 用户最近n次交互的物品id
- id embedding得到n个向量
- 将n个向量avg，作为用户的一种特征
- 这个特征可以用于召回双塔、粗排三塔、精排
- 可以分成点击的lastn、收藏的lastn、购买的lastn，分别算出来平均后的向量，再拼接起来
- attention比平均好
## DIN模型
- 使用加权平均，其实就是attention，权重是候选物品与lastN的相似度（这里的候选物品是指粗排筛出来的物品就是精排的候选物品）
- 可以将lastn作为key和value，将候选物品作为query，输入单层注意力层，能起到DIN模型一致的效果
- 注意力机制适用于精排模型，因为在召回和粗排中用户塔看不到候选物品，精排中物品和用户特征在一起
- 关注短期兴趣，遗忘长期兴趣
## SIM模型
- 长序列建模，保留用户的长期兴趣
- 给DIN加一个漏斗，在LastN中快速查找，找到k个相似物品
### 查找
- hard search：规则筛选，根据类目保留类目相同的，不用训练
- soft search: embedding,将候选物品作为query，knn查找相似度，找到topK
### 注意力
- 使用topK而不用lastN
- trick：使用时间信息，将用户与lastN物品交互时间距今时间离散化为区间，embedding，拼接物品向量，作为物品向量表征（有点类似于transform中使用位置编码）
- DIN序列短不考虑时间，SIM序列长考虑时间，时间越久重要性越低
# 重排
